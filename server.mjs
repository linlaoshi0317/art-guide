...
function buildAdaptiveGuidanceImagePrompt(styleGuide, variant = 1, talentType = null, note = "") {
  ...
  return [
    "🚨 核心指令：你是在优化一幅孩子的手绘作品，不是在重新创作。原画是神圣不可侵犯的基础。",
    "",
    "=== 最高优先级：原画100%保留 ===",
    "1. 原画中的每一根线条、每一个色块、每一个形状都必须原封不动地保留在最终画面中。",
    "2. 不得擦除、移动、缩放、旋转、替换、简化、美化、覆盖原画的任何部分。",
    "3. 不得改变原画中人物/动物/物体的数量、位置、大小比例关系。",
    "4. 原画的笔触质感（蜡笔/铅笔/水彩笔痕迹）必须清晰可见。",
    "5. 最终效果必须让人一眼认出：这就是孩子原来画的那张画。",
    "",
    "=== 内容添加：强关联原则 ===",
    "【禁止凭空添加】绝对不能添加与原画内容无关的通用元素。严禁添加以下'万能背景'：",
    "- 蓝天白云太阳（除非孩子原画中已经有）",
    "- 绿草地和花朵（除非孩子原画主题就是花园）",
    "- 远处的山和树（除非孩子原画中有相关元素）",
    "",
    "【必须强关联】只添加与原画已有内容直接相关的元素：",
    "- 如果孩子画了机器人 → 只能添加机械零件、齿轮、操作面板、科技背景",
    "- 如果孩子画了花朵 → 只能添加花园、草地、蝴蝶、浇水壶、花盆",
    "- 如果孩子画了恐龙 → 只能添加火山、原始森林、其他恐龙、化石",
    "- 如果孩子画了人物 → 只能添加与该人物直接相关的服装细节、手持物品、所在场景",
    "- 如果孩子画了汽车 → 只能添加道路、交通标志、车库、加油站",
    "- 如果孩子画了海洋动物 → 只能添加海水、珊瑚、气泡、其他海洋生物",
    "",
    "=== 构图优化：稳定画面结构 ===",
    "在保留原画所有内容的基础上，进行以下构图优化（不能改变原画主体位置）：",
    "- 如果原画主体偏左 → 在右侧添加回应元素（与原画内容强关联）",
    "- 如果原画主体偏右 → 在左侧添加回应元素",
    "- 如果原画主体偏上 → 在下方添加地基或陪衬元素",
    "- 如果原画空白过多 → 在空白处添加与主体直接相关的小元素（道具、伙伴、环境细节）",
    "- 如果原画较满 → 不做大幅添加，只补充最必要的背景层次",
    "- 添加前景/中景/背景层次，但不移动原画主体",
    "",
    "=== 风格统一：像孩子自己画的 ===",
    "- 添加的元素必须用与原画完全相同的手绘风格：歪歪扭扭的线条、不均匀的涂色、孩子气的比例",
    "- 不要画成成人插画风格、不要过度精致、不要完美对称",
    "- 颜色保持孩子级别的饱和度和平涂感，不要做复杂的渐变和阴影",
    "- 线条粗细和力度要与原画一致",
    "",
    `风格方向: ${style.styleCategory} (${style.styleName})`,
    `为什么适合: ${style.reason}`,
    `场景方向: ${style.sceneDirection}`,
    `色彩方向: ${style.colorDirection}`,
    `构图方向: ${style.compositionDirection}`,
    talentGuidance,
    "",
    "🚫 严禁：透明叠加效果 / 双曝光 / 前后对比面板 / 文字水印 / 教学标注 / 分屏布局 / 描图纸效果",
    "最终输出：一张完整的孩子手绘风格优化图，像孩子自己在老师引导下画完的完整作品。",
    note ? `\n=== 用户补充说明（最高优先级）===\n"${note}"\n请严格按此说明执行。` : "",
  ].filter(Boolean).join("\n");
}
