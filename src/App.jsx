        ...
        {gStatus !== "idle" && <div style={st.card}><div style={st.stepLabel}><span style={st.stepNum}>3</span> 对比结果 {gStatus === "done" ? "✓" : gStatus === "error" ? "✗" : "…"} {sName !== "自动匹配画风" ? `· ${sName}` : ""}</div>
          {gResults.length > 0 && gResults[0].colorized ? <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
              <div style={{ textAlign: "center" }}><div style={{ ...st.compareLabel, color: "#b8a99a", marginBottom: 4 }}>📐 优化线稿</div><img src={gResults[0].lineArtImage} alt="" style={{ width: "100%", borderRadius: 14, cursor: "pointer", objectFit: "contain", maxHeight: 260 }} onClick={() => setPreviewImage({ src: gResults[0].lineArtImage, title: "优化线稿" })} /><a href={gResults[0].lineArtImage} download="优化线稿.png" style={{ display: "inline-block", marginTop: 4, fontSize: 10, color: "#9b8c7c", textDecoration: "none" }} onClick={e => e.stopPropagation()}>💾 保存</a></div>
              <div style={{ textAlign: "center" }}><div style={{ ...st.compareLabel, color: "#E07B39", marginBottom: 4 }}>🎨 上色效果</div><img src={gResults[0].image} alt="" style={{ width: "100%", borderRadius: 14, cursor: "pointer", objectFit: "contain", maxHeight: 260 }} onClick={() => setPreviewImage({ src: gResults[0].image, title: "上色效果" })} /><a href={gResults[0].image} download="上色效果.png" style={{ display: "inline-block", marginTop: 4, fontSize: 10, color: "#9b8c7c", textDecoration: "none" }} onClick={e => e.stopPropagation()}>💾 保存</a></div>
            </div>
