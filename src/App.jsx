...
export function App() {
  ...
  async function runColorize() { if (gResults.length === 0) return; setCStatus("colorizing"); try { const lineArt = gResults[0].image; const data = await API("/api/colorize-image", { image: lineArt }); let coloredImg = data.image; const laSz = await GSZ(lineArt); if (laSz) coloredImg = await RESIZE(coloredImg, laSz.w, laSz.h); const newResults = [{ ...gResults[0], image: coloredImg, lineArtImage: lineArt, colorized: true }]; setGResults(newResults); setCStatus("done"); setGHistory(h => { const updated = [...h]; if (updated.length > 0) updated[0] = { ...updated[0], generatedImage: coloredImg, lineArtImage: lineArt }; return updated; }); } catch (e) { setCStatus("idle"); alert("上色失败：" + (e.message || "请重试")); } }
