...
export function App() {
  ...
  function handleFile(e) { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = () => { const src = String(r.result); cfRef.current = { name: f.name, src }; setPreview(src); setFileName(f.name.replace(/\.[^.]+$/, "") || C.newArtwork); setAnalysis(DA); setStatus("idle"); setASrc("idle"); setGResults([]); setGErr(""); setGStatus("idle"); setStyleGuide(null); gVar.current = 0; isLineArt(src).then(la => { setImageIsColor(!la); setEnableColoring(!la); }); runAnalysis(src, cfRef.current); }; r.readAsDataURL(f); }
