...
export function App() {
  ...
  function isLineArt(imgSrc) { return new Promise((resolve) => { const img = new Image(); img.onload = () => { const c = document.createElement("canvas"); c.width = 96; c.height = 96; const ctx = c.getContext("2d"); ctx.drawImage(img, 0, 0, 96, 96); const d = ctx.getImageData(0, 0, 96, 96).data; let sat = 0, count = 0; for (let i = 0; i < d.length; i += 16) { const r = d[i], g = d[i+1], b = d[i+2]; const mx = Math.max(r,g,b), mn = Math.min(r,g,b); if (mx > 30) { sat += mx === 0 ? 0 : (mx-mn)/mx; count++; } } resolve(count > 0 ? (sat/count) < 0.15 : true); }; img.onerror = () => resolve(false); img.src = imgSrc; }); }
