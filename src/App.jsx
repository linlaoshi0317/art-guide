...
export function App() {
  ...
  const [imageIsColor, setImageIsColor] = useState(false);
  ...
  async function runGuidance() { if (!preview || gStatus === "generating") return; const vb = ++gVar.current, seq = ++gSeq.current; const vars = Array.from({ length: GBS }, (_, i) => vb + i); gVar.current += GBS - 1; setGStatus("generating"); setGErr(""); setGResults([]); setStyleGuide(null); setShowColorizeBtn(false); setCStatus("idle"); try { const du = await TDU(preview); const sp = selStyle.id === "auto" ? null : selStyle; const results = await Promise.allSettled(vars.map(v => API("/api/generate-guidance-image", { fileName: cfRef.current.name || C.newArtwork, image: du, stylePreset: sp, variant: v, talentType: null, enableColoring, note: gNote }).then(async d => { let img = d.image; if (d.originalWidth && d.originalHeight) img = await FIA(img, d.originalWidth, d.originalHeight); return { image: img, model: d.model, styleGuide: d.styleGuide || null, variant: d.variant || v, originalWidth: d.originalWidth, originalHeight: d.originalHeight }; }))); if (gSeq.current !== seq) return; const ok = results.filter(r => r.status === "fulfilled").map(r => r.value); if (ok.length > 0) { setGResults(ok); setStyleGuide(ok[0].styleGuide || null); setGStatus("done"); runAnalysis(ok[0].image || preview, cfRef.current); const entryBase = { id: `${Date.now()}`, createdAt: new Date().toLocaleString("zh-CN", { hour: "2-digit", minute: "2-digit" }), fileName: fileName || C.newArtwork, preview, generatedImage: ok[0].image, styleName: SDN(ok[0].styleGuide || null) }; setGHistory(h => [entryBase, ...h].slice(0, 50)); if (enableColoring) { setCStatus("colorizing"); API("/api/colorize-image", { image: ok[0].image }).then(async data => { let ci = data.image; if (ok[0].originalWidth && ok[0].originalHeight) ci = await FIA(ci, ok[0].originalWidth, ok[0].originalHeight); const newResults = [{ ...ok[0], image: ci, lineArtImage: ok[0].image, colorized: true }]; if (gSeq.current === seq) { setGResults(newResults); setCStatus("done"); setGHistory(h => { const u = [...h]; if (u.length > 0) u[0] = { ...u[0], generatedImage: ci, lineArtImage: ok[0].image }; return u; }); } }).catch(() => { setCStatus("idle"); setShowColorizeBtn(true); }); } else { setShowColorizeBtn(true); } } else { const fe = results.find(r => r.status === "rejected"); setGErr(fe?.reason?.message || C.guideFailedHint); setGStatus("error"); } } catch (e) { if (gSeq.current === seq) { setGErr(e.message || C.guideFailedHint); setGStatus("error"); } } }

  function handleFile(e) { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = () => { const src = String(r.result); cfRef.current = { name: f.name, src }; setPreview(src); const rawName = f.name.replace(/\.[^.]+$/, "") || C.newArtwork; const cleanName = /^(screenshot|IMG_|mmexport|wx_camera|Screenshot|MicroMsg|com\.|Image_)/.test(rawName) ? C.newArtwork : rawName; setFileName(cleanName); setAnalysis(DA); setStatus("idle"); setASrc("idle"); setGResults([]); setGErr(""); setGStatus("idle"); setStyleGuide(null); gVar.current = 0; isLineArt(src).then(la => { setImageIsColor(!la); setEnableColoring(!la); }); runAnalysis(src, cfRef.current); }; r.readAsDataURL(f); }
  ...
  async function runColorize() { if (gResults.length === 0) return; setCStatus("colorizing"); try { const lineArt = gResults[0].image; const data = await API("/api/colorize-image", { image: lineArt }); let coloredImg = data.image; if (gResults[0].originalWidth && gResults[0].originalHeight) coloredImg = await FIA(coloredImg, gResults[0].originalWidth, gResults[0].originalHeight); const newResults = [{ ...gResults[0], image: coloredImg, lineArtImage: lineArt, colorized: true }]; setGResults(newResults); setCStatus("done"); setGHistory(h => { const updated = [...h]; if (updated.length > 0) updated[0] = { ...updated[0], generatedImage: coloredImg, lineArtImage: lineArt }; return updated; }); } catch (e) { setCStatus("idle"); alert("上色失败：" + (e.message || "请重试")); } }
  ...
  return (
    ...
    {!authToken ? <div style={{ ...st.container, maxWidth: 420, paddingTop: 80 }}>
      ...
      {activeTab === "analysis" && <div>
        ...
        <div style={st.card}><div style={st.stepLabel}><span style={st.stepNum}>2</span> 画风 & 上色选项</div><select value={styleId} onChange={e => { setStyleId(e.target.value); setGResults([]); setGErr(""); setGStatus("idle"); setStyleGuide(null); }} style={st.select}>{styleGroups.map(g => <optgroup key={g.label} label={g.label}>{g.options.map(o => <option key={o.id} value={o.id}>{o.label}</option>)}</optgroup>)}</select><p style={{ fontSize: 12, color: "#888", margin: "0 0 12px" }}>{selStyle.summary}</p>
          {/* 上色开关 */}
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button onClick={() => setEnableColoring(true)} style={{ flex: 1, padding: 12, borderRadius: 12, border: enableColoring ? "2px solid #E07B39" : "2px solid #e8e0d5", background: enableColoring ? "#fdf3ea" : "#fdfcf9", cursor: "pointer", textAlign: "left", transition: "all 0.2s" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: enableColoring ? "#E07B39" : "#5c4a3a", marginBottom: 2 }}>🎨 补全+上色</div>
              <div style={{ fontSize: 11, color: "#9b8c7c" }}>补全内容并添加色彩</div>
            </button>
            <button onClick={() => setEnableColoring(false)} style={{ flex: 1, padding: 12, borderRadius: 12, border: !enableColoring ? "2px solid #E07B39" : "2px solid #e8e0d5", background: !enableColoring ? "#fdf3ea" : "#fdfcf9", cursor: "pointer", textAlign: "left", transition: "all 0.2s" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: !enableColoring ? "#E07B39" : "#5c4a3a", marginBottom: 2 }}>✏️ 仅补全线稿</div>
              <div style={{ fontSize: 11, color: "#9b8c7c" }}>只补内容，不上色</div>
            </button>
          </div>
          {preview && <p style={{ fontSize: 11, color: "#E07B39", margin: "0 0 10px", fontWeight: 600 }}>{imageIsColor ? "已检测到彩色画面，默认「补全+上色」" : "已检测到线稿，默认「仅补全线稿」"}</p>}
          ...
          <button onClick={runGuidance} disabled={gStatus === "generating" || !preview || !childAge} style={gStatus === "generating" || !preview || !childAge ? st.btnDisabled : st.btnPrimary}>{!childAge && preview ? "请先选择年龄段" : gStatus === "generating" ? <><RefreshCcw size={18} className="spinning" />AI 优化中...</> : <><Sparkles size={18} />{enableColoring ? C.guideAction : "一键补全线稿"}</>}</button></div>
        ...
        {cStatus === "colorizing" && <div style={{ ...st.card, textAlign: "center", background: "#f9f6ff", border: "1px solid #e8dcf8" }}><p style={{ ...st.textSmall, margin: 0 }}><RefreshCcw size={16} className="spinning" style={{ marginRight: 6 }} />正在自动上色中……</p></div>}
        {showColorizeBtn && !gResults[0]?.colorized && <div style={st.card}><p style={{ ...st.textSmall, textAlign: "center", marginBottom: 12 }}>线稿已生成，需要上色吗？</p><button onClick={runColorize} disabled={cStatus === "colorizing"} style={st.btnAccent}>{cStatus === "colorizing" ? <><RefreshCcw size={16} className="spinning" />上色中...</> : <><Palette size={16} />一键上色</>}</button></div>}
