...
export function App() {
  ...
  const a = analysis, pa = a.psychologyAnalysis, fe = a.familyEducation, pr = a.projectionAnalysis, ti = a.talentInsight;
  ...
  return (
    <main style={st.page}><div style={st.container}>
      ...
      {activeTab === "analysis" && <div>
        <div style={st.card}><div style={st.stepLabel}>1 上传作品</div><label style={{ display: "block", cursor: "pointer" }}>{preview ? <img src={preview} alt="" style={st.img} /> : <div style={st.placeholder}>上传作品</div>}<input type="file" accept="image/*" onChange={handleFile} style={{ display: "none" }} /></label>
          <div style={{ marginTop: 12 }}><span style={{ fontSize: 12, color: "#E07B39" }}>*必选 </span><select value={childAge} onChange={e => setChildAge(e.target.value)} style={{ ...st.select, width: "auto", marginBottom: 0 }}><option value="">选择年龄段</option><option value="3-5">3~5岁</option><option value="5-8">5~8岁</option><option value="8-12">8~12岁</option><option value="12+">12岁+</option></select></div></div>
        ...
        {(status === "done" || aSrc !== "idle") && <div>
          {a.teacherCopy ? <div style={st.card}><h3 style={st.h3}>📝 老师点评</h3><p style={st.text}>{a.teacherCopy}</p></div> : (aSrc === "local" && <div style={st.card}><p style={{ ...st.text, textAlign: "center" }}>AI 分析暂不可用，请稍后重试。本地分析开发中。</p></div>)}
    ...
    {showReport && <div style={st.overlay} onClick={() => setShowReport(false)}><div style={st.modal} onClick={e => e.stopPropagation()}><div style={st.modalScroll} ref={rptRef}><div style={{ textAlign: "center", marginBottom: 20 }}><h1 style={{ fontSize: 22, margin: 0 }}>{C.reportTitle}</h1><p style={st.textSmall}>{fileName || C.newArtwork} · 年龄:{childAge || "未选择"}</p></div>
